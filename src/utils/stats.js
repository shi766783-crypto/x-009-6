...
export function getDashboardStats(items) {
  ...
  const warrantyCount = activeItems.filter((item) => getWarrantyState(item) === 'ok').length;
  const expiringCount = activeItems.filter((item) => getWarrantyState(item) === 'expiring').length;
